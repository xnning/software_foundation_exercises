...
Proof.
  intros b e1 e2 Hb st.
  split; intro H.
  Case "->".
    inversion H; subst.
    apply E_IfFalse; auto. simpl. rewrite H5. reflexivity.
    apply E_IfTrue; auto. simpl. rewrite H5. reflexivity.
  Case "<-".
    inversion H; subst.
    apply E_IfFalse; auto.
      simpl in H5.
      apply f_equal with (f:=negb) in H5.
      simpl in H5. rewrite negb_involutive in H5. assumption.
    apply E_IfTrue; auto.
      simpl in H5.
      apply f_equal with (f:=negb) in H5.
      simpl in H5. rewrite negb_involutive in H5. assumption.
...
Proof.
  ...
  split; intro Hy.
  apply WHILE_true_nonterm in Hy; auto.
    inversion Hy.
  apply WHILE_true_nonterm in Hy; auto.
    inversion Hy.
    unfold bequiv.
   ...
   split; intro Hy.
     Case "->".
       ...
       assert (st' = (update st' X (st' X))).
          ...
          rewrite update_same; reflexivity.
       rewrite H0 at 2.
       constructor. unfold aequiv in H.
       specialize (H st').
       simpl in H. auto.
     Case "<-".
       inversion Hy; subst.
       unfold aequiv in H.
       specialize (H st). simpl in H.
       rewrite <- H in Hy.
       assert (st = (update st X (st X))).
          apply functional_extensionality. intro.
          rewrite update_same; reflexivity.
