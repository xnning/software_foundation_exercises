...
Proof.
  ...
  assert(update (update st X (aeval st a)) X m = st).
    apply functional_extensionality. intros.
    rewrite update_shadow. apply update_same; auto.
...
Proof.
  ...
  eapply hoare_consequence_pre.
  apply hoare_asgn.
  unfold assert_implies . intros. split; reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  Case "THEN".
    ...
    intros. apply le_plus_minus. destruct H as [_ ?].
    apply ble_nat_true in H. auto.
  Case "ELSE".
    eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, update, assert_implies. simpl.
    reflexivity.
...
Proof.
  ...
  Case "True".
    eapply hoare_consequence_pre.
    eapply hoare_asgn.
    unfold assert_implies, assn_sub, update. intros. simpl.
    destruct H as [? _]. assumption.
  Case "False".
    eapply hoare_consequence_pre.
    eapply hoare_skip.
    unfold assert_implies, assn_sub, update, bassn, beval. intros.
    destruct H as [? ?].  destruct (beq_nat (st Y) 0) eqn:eqy.
      apply beq_nat_true in eqy. rewrite eqy in H. omega.
      unfold not in H0. simpl in H0.
      apply False_ind. apply H0. rewrite eqy. reflexivity.
Qed.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_RepeatEnd : forall e1 b1 st st',
      ceval st e1 st' ->
      beval st' b1 = true ->
      ceval st (REPEAT e1 UNTIL b1 END) st'
  | E_RepeatLoop : forall e1 b1 st st' st'',
      ceval st e1 st' ->
      beval st' b1 = false ->
      ceval st' (REPEAT e1 UNTIL b1 END) st'' ->
      ceval st (REPEAT e1 UNTIL b1 END) st''
...
Proof.
  ...
  ceval_cases (induction He) Case; intros;
    try (inversion Heqrcom); subst; clear Heqrcom.
  Case "E_RepeatEnd".
    split. eapply Hhoare1; eassumption. 
    apply bexp_eval_true. assumption.
  Case "E_RepeatLoop".
    eapply IHHe2; auto.
      exact Hhoare2.
      split. apply (Hhoare1 st st'); auto.
